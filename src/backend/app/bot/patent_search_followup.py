...
_DEFAULT_PRESENTATION_NUDGE = (
    "（用户看不到本条消息，请帮我把检索结果格式化展示给用户。）\n"
    "严格按以下结构生成回复，只生成一遍，绝对不要重复：\n\n"
    "1. 第一行：🔍 搜索完成\n"
    "2. 第二行：📊 找到 {总条数} 条专利（总条数取自工具结果）\n"
    "3. 空一行\n"
    "4. 制表符分隔表格，表头："
    "序号\\t专利号\\t标题\\t申请人\\t申请日\\t公开日\n"
    "   逐行展示工具结果中的专利；日期格式 YYYY-MM-DD；若无则留空\n"
    "5. 空一行\n"
    "6. 一行：📄 当前仅展示前 {本页条数} 条，共 {总条数} 条匹配。\n"
    "7. 小标题「初步观察」，接着 4–6 条要点（列表），概括申请人类型、"
    "代表性机构、技术方向、总量\n"
    "8. 小标题「后续操作」，用 action 链接格式列出：\n"
    "   - 翻页：[下一页](action:下一页) | [第3页](action:第3页)\n"
    "   - 排序：[最新公开](action:按公开日排序) | [最新申请](action:按申请日排序)\n"
    "   - 统计：[申请人分布](action:分析申请人分布) | [IPC分布](action:分析IPC分布)\n"
    "   - 导出：[导出Excel](action:导出当前搜索结果)\n"
    "   - 详情：直接说专利号，如「CN115755970A的详情」\n"
    "   - 企业画像：直接说企业名，如「广州极飞科技的企业画像」\n"
    "⚠️ 绝对禁止：不要重复生成表格！表格以上述格式出现一次后，"
    "不要再以任何形式再次列出专利号或表格。\n"
    "不要输出命令原文（command= / page= 等技术参数）。"
    "操作建议必须用 [操作名](action:用户要说的话) 格式。"
)
