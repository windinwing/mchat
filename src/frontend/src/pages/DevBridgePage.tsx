        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-4">
          <Card className="xl:col-span-3">
            <CardHeader className="pb-2">
              <div className="text-sm font-semibold text-gray-900 dark:text-gray-100">{t('devbridge.providers')}</div>
            </CardHeader>
              ...
              {providers.map((provider) => (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-medium text-gray-900 dark:text-gray-100">{provider.title}</span>
                    <Badge variant={provider.enabled ? 'success' : 'default'}>
                      {provider.enabled ? t('common.enabled') : t('common.disabled')}
                    </Badge>
                  </div>
                  <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">{provider.key}</div>
                  {provider.capabilities.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2">
                      {provider.capabilities.slice(0, 4).map((cap) => (
                        <span key={cap} className="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300">
                          {cap}
                        </span>
                      ))}
                    </div>
                ...
                <div className="space-y-2 max-h-[520px] overflow-y-auto">
                  ...
                  {filteredProjects.map((project) => (
                    ...
                    >
                      ...
                      <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{project.slug}</div>
                      <div className="flex items-center gap-2 mt-1">
                        <Badge variant={project.has_build ? 'success' : 'default'}>
